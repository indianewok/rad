...
static void usage(const char* prog) {
    std::cerr
      << "Usage: " << prog
      << " -l LAYOUT -q FASTQ [-k KIT] [-w PATH] [-o PREFIX] [-d DIR] [-t THREADS] [-v]\n\n"
      << "  -l, --layout     layout key (five_prime|three_prime|splitseq)\n"
      << "  -q, --fastq      input FASTQ file\n"
      << "  -k, --kit        use only this kit's default whitelist\n"
...
int main(int argc, char* argv[]) {
    std::string layout_key, fastq_path;
    std::string kit, whitelist_path;
    std::string output_prefix, output_dir;
    int num_threads = 1;
    bool verbose = false;
    bool split_bc = false;

    const char* optstring = "l:q:k:w:o:d:t:vh";
    ...
    try {
        ...
        auto layout_csv = config_utils::get_read_layout(layout_key);
        ...
        if (have_layout) {
            ...
            read_layout.import_read_layout(outbase.string() + "_layout.csv", verbose);
        } else {
            if (verbose) std::cout << "[main] Generating read layout...\n";
            read_layout.prep_new_layout(layout_csv);
        }

        if (have_map) {
            if (verbose) std::cout << "[main] Importing position map from " << outbase.string() + "_position_map.csv" << "\n";
            read_layout.import_position_map(outbase.string() + "_position_map.csv", verbose);
        } else {
        ...
        SigString::sigalign(fastq_path, read_layout, outbase.string(), num_threads, /*max_reads=*/-1, /*verbose=*/verbose);

        // save whitelist summary
        if (verbose) std::cout << "[main] Saving whitelist summary to " << outbase.string() + "_whitelist.csv" << "\n";
        read_layout.save_wl(outbase.string() + "_whitelist.csv", /*full=*/true);
    }
