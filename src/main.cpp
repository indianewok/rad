...
static void print_main_usage(const char *prog) {
  std::cerr << "Usage: " << prog << " <command> [options]\n\n"
            ...
            << "  reformat          Reformat fastx.gz outputs for downstream "
               "applications\n"
            << "  list              List available read layouts and/or "
               "whitelist kits\n"
...
static void usage_demux(const char *prog) {
  std::cerr
      ...
      << "Required:\n"
      << "  -l, --layout                      layout key (five_prime, "
         "three_prime, splitseq) or path\n"
...
static void usage_reformat(const char *prog) {
  std::cerr << "Usage: rad " << prog << " -q INPUT [options]\n\n"
            << "Options:\n"
            ...
            << "      --reformat-header collapse headers to QNAME_CB_UB "
               "(default underscore)\n"
            << "      --reformat-delim  delimiter for collapsed headers "
            ...
            << "  -h, --help            show this help\n\n"
            << "If both --split-bc and --reformat-header are set, headers are "
...
static void usage_prep(const char *prog) {
  std::cerr
      ...
      << "Generate read layout structure and/or position map with misalignment "
         "statistics.\n\n"
      << "Required:\n"
      << "  -l, --layout                      layout key or path to layout "
         "CSV\n\n"
      << "Mode flags (at least one required):\n"
      << "      --read-layout                 generate and display read layout "
         "structure\n"
      << "      --position-map                generate position map with "
         "misalignment stats\n"
      << "                                    (requires -q/--fastq)\n\n"
      << "Optional:\n"
      << "  -q, --fastq                       input FASTQ file (required for "
         "--position-map)\n"
      ...
      << "  rad " << prog
      << " -l my_layout.csv --position-map -q reads.fq.gz -o output\n"
      << "  rad " << prog
      << " -l five_prime --read-layout --position-map -q reads.fq.gz -o "
         "output\n";
}
...
static void usage_list(const char *prog) {
  std::cerr
      ...
      << "List registered layout keys and whitelist kit keys with resolved "
         "paths.\n\n"
      << "Options:\n"
      << "      --layouts                    list read layout keys and paths\n"
      << "      --whitelists                 list whitelist kit keys and "
         "paths\n"
      << "  -h, --help                       show this help\n\n"
      << "If neither --layouts nor --whitelists is provided, both are "
         "listed.\n";
}
...
public:
  tee_buff(std::streambuf *sb1, std::streambuf *sb2) : sb1(sb1), sb2(sb2) {}
...
int cmd_prep(int argc, char *argv[]) {
  ...
  try {
    ...
    if (!config_utils::check_if_custom_rl(layout_key)) {
      ...
      if (verbose)
        std::cout << "[prep] Using built-in layout: " << layout_key << "\n";
    } else {
    ...
    if (do_position_map) {
      ...
      // Write updated files
      if (verbose)
        std::cout << "[prep] Writing output files...\n";
...
static std::optional<std::string> extract_cb(const std::string &id,
...
parse_collapsed_id(const std::string &id, char sep) {
  ...
  if (qname.empty() || cb.empty() || ub.empty())
    return std::nullopt;
  return std::make_tuple(std::move(qname), std::move(cb), std::move(ub));
}

static std::string make_visium_hd_comment(const std::string &bc,
                                          const std::string &cb,
                                          const std::string &ub,
...
static bool parse_legacy_bin_token(const std::string &token,
                                   int &out_bin_size_um) {
  // Accept either integer microns (e.g. "8") or legacy token "s008um".
  if (parse_positive_int(token, out_bin_size_um))
    return true;
...
load_barcode_csv_first_column(const std::string &path) {
  ...
  for (auto &line : lines) {
    ...
    if (first) {
      first = false;
      if (to_lower_copy(trimmed) == "whitelist_bcs")
        continue;
    }
...
static vizhd_axis_reference load_vizhd_v1_axis_reference() {
  ...
  ref.x_lengths.erase(std::unique(ref.x_lengths.begin(), ref.x_lengths.end()),
                      ref.x_lengths.end());
  std::sort(ref.y_lengths.begin(), ref.y_lengths.end());
  ref.y_lengths.erase(std::unique(ref.y_lengths.begin(), ref.y_lengths.end()),
                      ref.y_lengths.end());
  return ref;
}

static std::optional<std::pair<std::string, std::string>>
resolve_xy_barcodes_from_cb(const std::string &cb_tag,
                            const vizhd_axis_reference &ref) {
  if (cb_tag.empty())
    return std::nullopt;
  ...
  for (int lx : ref.x_lengths) {
    for (int ly : ref.y_lengths) {
      if (static_cast<int>(cb_tag.size()) != lx + ly)
        continue;

      std::string xs = cb_tag.substr(0, static_cast<size_t>(lx));
      std::string ys =
          cb_tag.substr(static_cast<size_t>(lx), static_cast<size_t>(ly));
...
static std::string make_spatial_mapping_id(int bin_size_um, int x_bin,
                                           int y_bin) {
  std::ostringstream oss;
...
static std::string make_coordinate_read_id(const std::string &base_id,
                                           int x_bin, int y_bin) {
...
int cmd_reformat(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv, optstring, longopts, nullptr)) != -1) {
    ...
    case 5: {
      ...
      if (!parse_positive_int(optarg ? std::string(optarg) : "", parsed)) {
        std::cerr
            << "[ERROR] --bin-size must be a positive integer (microns)\n\n";
        usage_reformat(argv[0]);
        return 1;
      }
      bin_size_um = parsed;
    ...
    case 7: {
      ...
      if (!parse_legacy_bin_token(arg, parsed)) {
        std::cerr << "[ERROR] --visium-hd-sample expects legacy sXXXum token "
                     "or integer bin size\n\n";
        usage_reformat(argv[0]);
        return 1;
      }
  ...
  if (do_coordinate) {
    ...
    try {
      ...
      std::cerr << "[ERROR] Failed to load coordinate reference for mode "
                << coordinate_mode << ": " << e.what() << "\n";
      return 1;
    }
    if (verbose) {
  ...
  auto get_writer = [&](const std::string &bc) -> ref_writer & {
    ...
    if (w->pigz_fp) {
      ...
      if (!w->gz)
        throw std::runtime_error("Failed to open output " + p.string());
  ...
  auto open_single_writer =
      [&](const std::string &path) -> std::unique_ptr<ref_writer> {
  ...
  auto write_record = [&](ref_writer &w, const read_streaming::sequence &r,
                          bool use_lock) {
    std::string rec;
    if (r.is_fastq) {
      rec.reserve(r.id.size() + r.comment.size() + r.seq.size() +
                  r.qual.size() + 16);
      rec += '@';
      rec += r.id;
      if (!r.comment.empty()) {
  ...
  try {
    ...
    chunk_streaming<read_streaming::sequence,
                    std::function<void(std::vector<read_streaming::sequence> &,
                                       const std::string &)>>
        cs(5000);
    ...
    if (do_reformat && !do_split) {
      ...
      const bool in_is_gz =
          in_name.size() >= 3 && in_name.substr(in_name.size() - 3) == ".gz";
    ...
    auto chunk_func = [&](std::vector<read_streaming::sequence> &chunk,
                          const std::string &) {
      for (auto &r : chunk) {
        ...
        if (do_coordinate) {
          if (cb.has_value() && ub.has_value() && !cb->empty() &&
              !ub->empty()) {
            ...
            if (xy.has_value()) {
              ...
              if (xit != spatial_ref->x_index_by_bc.end() &&
                  yit != spatial_ref->y_index_by_bc.end()) {
                ...
                std::string spatial_bc =
                    make_spatial_mapping_id(bin_size_um, x_bin, y_bin);
                r.comment = make_visium_hd_comment(
                    spatial_bc, *cb, *ub, make_bin_rg_tag(bin_size_um));
              }
            } else if (verbose) {
              std::cout << "[reformat] skipped coordinate mapping for read "
                           "with unresolvable CB pair: "
                        << *cb << "\n";
            }
          } else if (verbose) {
            std::cout << "[reformat] skipped coordinate mapping for read "
                         "without parsable CB/UB: "
                      << r.id << "\n";
          }
        } else if (do_reformat) {
...
int cmd_demux(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv, optstring, longopts, nullptr)) != -1) {
    ...
    case 'k':
      ...
      break;
    case 'g':
      global_whitelist_path = optarg;
      break;
    case 'c':
      custom_whitelist_path = optarg;
      break;
    case 'R':
      bc_corr_mode = optarg;
      break;
    case 'M':
      wl_mut = std::stoi(optarg);
      break;
    case 'm':
      gen_mut = std::stoi(optarg);
      break;
    case 'n':
      max_reads = std::stoull(optarg);
      break;
    case 'z':
      chunk_size = std::stoull(optarg);
      break;
    case 'o':
    ...
    case 1: {
      ...
      std::transform(
          joint_bc_mode.begin(), joint_bc_mode.end(), joint_bc_mode.begin(),
          [](unsigned char ch) { return static_cast<char>(std::tolower(ch)); });
      if (joint_bc_mode == "auto")
        joint_bc_mode = "default";
      if (joint_bc_mode != "default" && joint_bc_mode != "strict") {
        std::cerr
            << "[ERROR] --joint-bc-mode must be one of: default, strict\n";
        return 1;
      }
      break;
    }
  ...
  if (!log_file.empty()) {
    ...
    if (max_verbose) {
      // In max verbose mode, send all stdout/stderr to the log file to avoid
      // flooding the terminal.
      std::cout.rdbuf(log_stream.rdbuf());
      std::cerr.rdbuf(log_stream.rdbuf());
      std::cerr << "[main] Max verbose output redirected to log file: "
                << log_file << "\n";
    } else {
      tee_buf =
          std::make_unique<tee_buff>(std::cout.rdbuf(), log_stream.rdbuf());
  ...
  if (!outbase_parent.empty() && !ensure_dir(outbase_parent)) {
    std::cerr << "[ERROR] Cannot create output directory: "
              << outbase_parent.string() << "\n";
    return 1;
  }

  std::string base = outbase.filename().string();
  std::string outdir_display =
      outbase_parent.empty() ? outdir.string() : outbase_parent.string();
  ...
  if (verbose) {
    std::cout << "============= Configuration =============\n"
              ...
              << "  custom kit                   : "
              << (custom_kit.empty() ? "[none]" : custom_kit) << "\n"
              << "  global whitelist             : "
              << (global_whitelist_path.empty() ? "[none]"
                                                : global_whitelist_path)
              << "\n"
              << "  custom whitelist             : "
              << (custom_whitelist_path.empty() ? "[none]"
                                                : custom_whitelist_path)
              << "\n"
              ...
              << (gen_mut ? std::to_string(*gen_mut) : "default(2)") << "\n"
              << "  max reads                    : "
              << (max_reads == -1 ? "all" : std::to_string(max_reads)) << "\n"
              << "  chunk size                   : " << chunk_size << "\n"
              << "  output directory             : " << outdir_display << "\n"
              << "  write debug files            : " << std::boolalpha
              << write_debug << "\n"
              << "  filename base                : " << base << "\n"
              << "  log file                     : "
              << (log_file.empty() ? "[none]" : log_file) << "\n"
              << "  barcode split                : " << std::boolalpha
              << split_bc << "\n"
              << "  threads                      : " << nthreads << "\n"
              << "  verbose                      : " << std::boolalpha
              << verbose << "\n"
  ...
  try {
    ...
    bool have_map =
        boost::filesystem::exists(outbase.string() + "_position_map.csv");
    ...
    if (!have_layout || !have_map) {
      ...
      std::cout << "[main] Misalignment time: "
                ...
                << " ms\n";
      if (verbose)
        std::cout << "[main] Writing updated layout & map...\n";
      if (max_verbose)
        read_layout.display_read_layout();
    ...
    // Whitelist loading
    std::string kit_or_wl =
        custom_kit.empty() ? global_whitelist_path : custom_kit;
    ...
    std::cout << "[main] Total code duration: "
              << std::chrono::duration_cast<std::chrono::seconds>(final_elapsed)
                     .count()
              << " s\n";
    ...
    if (split_bc) {
      if (verbose)
        std::cout << "[main] Splitting output by barcode...\n";
      auto split_start = std::chrono::steady_clock::now();
      try {
        // io_bc_split_utils::split_fastqas(outbase.string(), verbose,
        // nthreads);
        ...
        std::cerr << "[WARNING] Barcode splitting failed: " << ex.what()
                  << std::endl;
      }
    }
...
int cmd_list(int argc, char *argv[]) {
  ...
  struct option longopts[] = {{"layouts", no_argument, nullptr, 1},
...
int cmd_modify(int argc, char *argv[]) {
  ...
  const char *optstring = "h";
  struct option longopts[] = {{"layout", required_argument, nullptr, 1},
  ...
  if (target_layout) {
    if (do_add) {
      ...
      try {
        ...
        std::cout << "[modify] warning: layout is currently unresolved ("
                  << e.what() << ")\n";
      }
    } else {
    ...
    if (do_add) {
      ...
      try {
        ...
        std::cout << "[modify] warning: whitelist is currently unresolved ("
                  << e.what() << ")\n";
      }
    } else {
